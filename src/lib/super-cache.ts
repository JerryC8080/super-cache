...
function initStorage(ins, storage) {
  ...
  if (typeof storage.get !== 'function')
    throw new Error('storage.get must typeof function');
  if (typeof storage.set !== 'function')
    throw new Error('storage.set must typeof function');
  ins.getData = (key) => storage.get.call(ins, `${storage.keyPrefix}:${key}`);
  ins.setData = (key, value) =>
    storage.set.call(ins, `${storage.keyPrefix}:${key}`, value);
  ...
  if (typeof storage.remove !== 'function')
    ...
    ins.removeData = (key) =>
      storage.remove.call(ins, `${storage.keyPrefix}:${key}`);

  if (typeof storage.removeAll !== 'function')
    ins.removeAll = () => {
...
export class SuperCache {
  ...
  constructor({
    ...
    this.log = log;
    this.adapterGlobalOptions = Object.assign(
      {},
  ...
  get(key, options: any = {}) {
    ...
    return (
      Promise.resolve()
        ...
        // 获取 adapter
        .then(() => {
          domain.adapter = this.getAdapter(key);
        })

        // 处理 adapterOptions
        .then(() => {
          // 合并 adapterOptions
          domain.adapterOptions = Object.assign(
            {},
            this.adapterGlobalOptions,
            domain.adapter.options,
        ...
        .then(() => {
          ...
          if (
            typeof domain.cacheValue === 'undefined' ||
            domain.adapterOptions.ignoreCache === true
          ) {
          ...
          if (domain.adapterOptions.updateCache) {
            this.getAndUpdateAdapterValue(key, domain.adapterOptions)

              // 自己处理 error，不阻塞流程
              .catch((error) =>
                this.log.error(
                  'updateCache',
                  'getAndUpdateAdapterValue fail',
                  error
                )
              );
          }
        ...
        .catch((err) => {
          ...
          if (err.message === `adapter ${key} was undefined`)
            return Promise.resolve(domain.cacheValue);
  ...
  getAndUpdateAdapterValue(key, { updateCache = true }) {
    return this.getAdapterValue(key).then((value) => {
  ...
  addAdapter(key, adapter) {
    ...
    if (this.adapters[key])
      throw new Error(`adapter ${key} not allowed to be repeat definition`);
    if (typeof _adapter === 'function') _adapter = { data: _adapter };
    else if (typeof _adapter.data !== 'function')
      throw new Error('data() must typeof function');
